...
## 3. 핵심 트러블슈팅
Issue: Ubuntu 24.04 Nginx 설치 시 IPv6 소켓 에러로 인한 서비스 기동 실패.
Solution: /etc/nginx/sites-available/default 설정 수정을 통해 IPv4 환경에 최적화하여 해결.
Issue: NCP ASG의 단일 서브넷 선택 제약으로 인한 멀티 존 구성의 어려움.
Solution: Zone별로 독립된 ASG를 생성하고 하나의 Target Group에 바인딩하는 Multi-ASG 구조로 고가용성 구현.
